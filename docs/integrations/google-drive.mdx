...
5. On the **Scopes** page, you may add desired scopes (see Supported Scopes below), but with OpenInt, begin with `https://www.googleapis.com/auth/drive.metadata.readonly` for metadata-only access. Click **Save and Continue**.
...
5. Add an **Authorized redirect URI** pointing to your OpenInt instance's callback endpoint; e.g.,
...
3. Set the scope to `https://www.googleapis.com/auth/drive.metadata.readonly` (default and currently supported by OpenInt).
...
> For scope details and requirement levels (sensitive/restricted), see Google’s [full scope list](https://developers.google.com/workspace/drive/api/guides/api-specific-auth#drive-scopes).
